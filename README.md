...
Combat Commander 앱은 한국어 규칙 V3.1과 제공된 명령 보드·행동 표를 따라 봇 준비, 명령·행동 판정, 트랙 이동, 턴 종료를 안내합니다. 원작 CC: BOT 제작자는 **jamesWUK**입니다. 실제 유닛의 이동·공격 대상은 플레이어가 결정합니다. 개별 행동 이름은 제공된 영문 표기를 유지합니다. 명령·행동 트랙 끝 이후 이동은 제공 자료에 없으므로 앱이 표시 가능한 마지막 칸을 유지합니다.
...
운명 카드를 뽑으면 카드 이미지의 흰색·빨간색 주사위 값이 자동으로 표시되고 판정에 사용됩니다. 규칙서의 **“검은색 주사위”는 제공된 카드 이미지의 빨간색 주사위**입니다. 미국 일반 시트의 마지막 뒷면 칸은 뽑기 대상에서 제외했습니다.

봇 운명 덱의 마지막 카드를 뽑으면 사용자가 제공한 기본 게임의 **시간! 절차**를 시작합니다. 시간 마커를 한 칸 전진한 뒤 봇 덱 재구성, 돌연 종료 확인, 방어자 VP, 연막 제거, 증원 배치, 양쪽 진지 구축을 순서대로 안내합니다. 사용자가 실제 게임판의 현재 시간 칸과 돌연 종료 마커 칸을 입력하면, 해당할 때 섞은 봇 운명 덱에서 한 장을 뽑아 두 주사위 합으로 돌연 종료를 자동 판정합니다. [적용 근거](docs/COMBAT_COMMANDER.md).
...
[패스 오브 글로리 카드 110장 단일 JSON](docs/cards/paths_of_glory_all.json)에 진영·번호·단계·작전값·전략적 재배치값(SR)·전투 카드 여부·충원값(RP), 한글/영문 제목, 카드 문구 OCR, 원본 이미지 경로를 모았습니다. 카드 문구는 자동 판독 초안이므로 오탈자와 누락 가능성이 있습니다. 실제 카드 사용 조건과 효과는 JSON의 이미지 경로에 있는 원본을 확인하세요. 이 자료는 게임 규칙 엔진에 적용되지 않습니다.
